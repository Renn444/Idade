...
def main():
    ...
    def MTBOF(S, T, Z):
        ...
        def V2(S, T):
            return Rh(T - S) * (quad(lambda x: x * fx(x), S, T)[0]) + dblquad(lambda x, h: x * fh(h) * fx(x), 0, T - S, S,
                                                                           lambda h: S + h)[0]
    
        # CASO 3
        def P3(S, T, Z):
            return p * Rh(Z - S) * (Fx(Z) - Fx(T)) + p * (
                dblquad(lambda x, h: fh(h) * fx(x), T - S, Z - S, T, lambda h: h + S)[0])
    
        def C3(S, T, Z):
            return cf * P3(S, T, Z)
    
        def V3(S, T, Z):
            return p * Rh(T - S) * (quad(lambda x: x * fx(x), T, Z)[0]) + p * (
                dblquad(lambda x, h: x * fh(h) * fx(x), T - S, Z - S, T, lambda h: h + S)[0])
