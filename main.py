...
def main():
    ...
    if choice == menu[0]:
        ...
        beta = st.number_input('Parâmetro de forma (beta)', step=0.1, format='%.1f', min_value=2.0, max_value=5.0)
        eta = st.number_input('Parâmetro de escala (eta)', step=0.1, format='%.1f', min_value=3.0, max_value=4.0)
        lbda = st.number_input('Taxa de Chegada de Oportunidade (Lambda)', step=0.1, format='%.1f', min_value=0.0, max_value=4.0)
        cp = st.number_input('Custo de Substituição Preventiva em T(programado):', step=10.0, format='%.1f', min_value=1.0, max_value=100.0)
        cv = st.number_input('Custo de Substituição Preventiva em Z:', step=10.0, format='%.1f', min_value=1.0, max_value=3.0)
        co = st.number_input('Custo de Substituição Preventiva em Oportunidade:', step=10.0, format='%.1f', min_value=0.25, max_value=1.0)
        cf = st.number_input('Custo da manutenção corretiva:', step=0.01, format='%.2f', min_value=0.01, max_value=10.0)
        cw = st.number_input('Substituição oportuna entre T e Z:', step=10.0, format='%.1f', min_value=0.25, max_value=1.0)
        p = st.number_input('#Probabilidade de Impedimento:', step=0.01, format='%.2f', min_value=0.01, max_value=0.2)
