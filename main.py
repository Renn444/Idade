...
def MTBOF(S, T, Z, beta, eta, lbda, cf, co, cw, cp, cv, p):
    def fx(x): 
    ...
    def objetivo(y):
        ...
        SOMA_PROB_FALHAS = P1(S) + P2(S, T) + P3(S, T, Z) + P4(S, T) + P5(S, T, Z) + P6(S, T) + P7(S, T, Z)
...
def main():
    ...
    if choice == menu[0]:
        ...
        cp = st.number_input('Custo de Substituição Preventiva em T(programado):', value=100.0, step=10.0, format='%.1f')
        cv = st.number_input('Custo de Substituição Preventiva em Z:', value=100.0, step=10.0, format='%.1f')
        co = st.number_input('Custo de Substituição Preventiva em Oportunidade:', value=100.0, step=10.0, format='%.1f')
        ...
        if botao:
            S, T, Z, CR = MTBOF(beta, eta, lbda, cf, co, cw, cp, cv, p)
            st.write('S = :', S)
            st.write('T = :', T)
            st.write('Z = :', Z)
            st.write('Taxa de custo = :', CR) 
